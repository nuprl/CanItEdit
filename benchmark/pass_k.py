...
- MaxCompletions: the maximum number of completions for any problem in the
  directory
- ExcessCode: the mean median test coverage for the problems in the
  directory
"""
import numpy as np
...
def for_file(path):
    ...
    coverages = [r["coverage"] for r in data["results"] if r["status"]
...
def main():
    ...
    if not args.suppress_header:
        print(
            "Dataset,Pass@k,Estimate,NumProblems,MinCompletions,MaxCompletions,ExcessCode")
    for d in args.dirs:
        ...
        median_coverages = [r["median_coverage"]
                            for r in results if r["median_coverage"] is not None]
        if len(median_coverages) > 0:
            mean_median_coverage = np.mean(median_coverages)
            excess_code = 100-mean_median_coverage
        else:
            excess_code = "NA"
        ...
        if temperature == 0.8:
            ...
            print(
                f"{name},10,{pass_10},{num_problems},{min_completions},{max_completions},{excess_code}")
            print(
                f"{name},100,{pass_100},{num_problems},{min_completions},{max_completions},{excess_code}")
        else:
            pass_1 = np.mean([r["pass@1"] for r in results])
            print(
                f"{name},1,{pass_1},{num_problems},{min_completions},{max_completions},{excess_code}")

        if args.k is not None:
            pass_k = np.mean([estimator(r["n"], r["c"], args.k)
                             for r in results])
            print(
                f"{name},{args.k},{pass_k},{num_problems},{min_completions},{max_completions},{excess_code}")
